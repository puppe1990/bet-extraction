...
const appUrl = process.env.E2E_EXTENSION_APP_URL ?? "https://bankrollkit.netlify.app";
...
test("extension popup login smoke", async () => {
	...
	const userDataDir = mkdtempSync(path.join(os.tmpdir(), "bankrollkit-extension-"));
	...
	try {
		...
		await page.getByLabel("BankrollKit app URL").fill(appUrl);
